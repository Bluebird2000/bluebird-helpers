...
export const useDateUtils = () => {
  ...
  const getFullMonth = (index: number) => {
    const MONTHS = [
  ...
  const getDateInDDMMYYYY = (ts: number) => dayjs(ts).format("DD/MM/YYYY");

  const getDateInDDMonthYYYY = (ts: number) => dayjs(ts).format("DD/MMM/YYYY");

  const getDateInReverseKebabCase = (ts: number) => dayjs(ts).format("YYYY-MM-DD");

  const getDateInSentence = (ts: number) => {
